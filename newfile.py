...
class Assets(Main):
	...
	def dumpAccount(self,**wish_mekar_op):
		global data_id
		if wish_mekar_op['chose']=="friends":
			r = requests.get(f"{wish_mekar_op['url']}",cookies=self.coki).json()['friends']
		else:
			r = requests.get(f"{wish_mekar_op['url']}",cookies=self.coki).json()['subscribers']
		for x in r['data']:
			try:
